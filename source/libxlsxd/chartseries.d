...
struct Chartseries {
	...
	void setCategories(string sheetName, lxw_row_t firstRow, lxw_col_t
			firstCol, lxw_row_t lastRow, lxw_col_t lastCol) @trusted
	{
	...
	void setLine(lxw_chart_line* line) nothrow @nogc @trusted {
		chart_series_set_line(this.handle, line);
	}

	void setFill(lxw_chart_fill* fill) nothrow @nogc @trusted {
		chart_series_set_fill(this.handle, fill);
	}

	void setInvertIfNegative() nothrow @nogc @trusted {
		chart_series_set_invert_if_negative(this.handle);
	}

	void setPattern(lxw_chart_pattern* pattern) nothrow @nogc @trusted {
		chart_series_set_pattern(this.handle, pattern);
	}

	void setMarkerType(ubyte mark) nothrow @nogc @trusted {
		chart_series_set_marker_type(this.handle, mark);
	}

	void setMarkerSize(ubyte size) nothrow @nogc @trusted {
		chart_series_set_marker_size(this.handle, size);
	}

	void setMarkerLine(lxw_chart_line* line) nothrow @nogc @trusted {
		chart_series_set_marker_line(this.handle, line);
	}

	void setMarkerFill(lxw_chart_fill* fill) nothrow @nogc @trusted {
		chart_series_set_marker_fill(this.handle, fill);
	}

	void setMarkerPattern(lxw_chart_pattern* pattern) nothrow @nogc @trusted {
		chart_series_set_marker_pattern(this.handle, pattern);
	}

	void setPoints(lxw_chart_point** point) @trusted {
		enforce(chart_series_set_points(this.handle, point)
				== LXW_NO_ERROR
			);
	}

	void setSmooth(ubyte smooth) nothrow @nogc @trusted {
		chart_series_set_smooth(this.handle, smooth);
	}

	void setLabels() nothrow @nogc @trusted {
		chart_series_set_labels(this.handle);
	}

	void setLabelsOptions(bool showName, bool showCategory ,
			bool showValue) @trusted
	{
	...
	void setLabelsSeparator(ubyte seperator) nothrow @nogc @trusted {
		chart_series_set_labels_separator(this.handle, seperator);
	}

	void setLabelsPosition(ubyte position) nothrow @nogc @trusted {
		chart_series_set_labels_position(this.handle, position);
	}

	void setLabelsLeaderLine() nothrow @nogc @trusted {
		chart_series_set_labels_leader_line(this.handle);
	}

	void setLabelsLegend() nothrow @nogc @trusted {
		chart_series_set_labels_legend(this.handle);
	}

	void setLabelsPercentage() nothrow @nogc @trusted {
		chart_series_set_labels_percentage(this.handle);
	}

	void setLabelsNumFormat(string format) @trusted {
		chart_series_set_labels_num_format(this.handle, toStringz(format));
	}

	void setLabelsFont(lxw_chart_font* font) nothrow @nogc @trusted {
		chart_series_set_labels_font(this.handle, font);
	}

	void setTrendline(ubyte type, ubyte value) nothrow @nogc @trusted {
	...
	void setTrendlineEquation() nothrow @nogc @trusted {
		chart_series_set_trendline_equation(this.handle);
	}

	void setTrendlineRsquared() nothrow @nogc @trusted {
		chart_series_set_trendline_r_squared(this.handle);
	}

	void setTrendlineIntercept(double intercept) nothrow @nogc @trusted {
		chart_series_set_trendline_intercept(this.handle, intercept);
	}

	void setTrendlineName(string name) @trusted {
		chart_series_set_trendline_name(this.handle, toStringz(name));
	}

	void setTrendlineLine(lxw_chart_line* line) nothrow @nogc @trusted {
