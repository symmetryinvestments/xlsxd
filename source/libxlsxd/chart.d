...
struct Chart {
	...
	void titleSetName(string title) nothrow {
	...
	void titleSetNameFont(lxw_chart_font* font) nothrow @nogc {
		chart_title_set_name_font(this.handle, font);
	}

	void titleOff() nothrow @nogc {
		chart_title_off(this.handle);
	}

	void legendSetPosition(ubyte pos) nothrow @nogc {
		chart_legend_set_position(this.handle, pos);
	}

	void legendSetFont(lxw_chart_font* font) nothrow @nogc {
		chart_legend_set_font(this.handle, font);
	}

	lxw_error legendDeleteSeries(short[] series) nothrow @nogc {
		return chart_legend_delete_series(this.handle, series.ptr);
	}

	void chartareaSetLine(lxw_chart_line* line) nothrow @nogc {
		chart_chartarea_set_line(this.handle, line);
	}

	void chartareaSetFill(lxw_chart_fill* fill) nothrow @nogc {
		chart_chartarea_set_fill(this.handle, fill);
	}

	void chartareaSetPattern(lxw_chart_pattern* pattern) nothrow @nogc {
		chart_chartarea_set_pattern(this.handle, pattern);
	}

	void plotareaSetLine(lxw_chart_line* line) nothrow @nogc {
		chart_plotarea_set_line(this.handle, line);
	}

	void plotareaSetFill(lxw_chart_fill* fill) nothrow @nogc {
		chart_plotarea_set_fill(this.handle, fill);
	}

	void plotareaSetPattern(lxw_chart_pattern* pattern) nothrow @nogc {
		chart_plotarea_set_pattern(this.handle, pattern);
	}

	void setStyle(ubyte style) nothrow @nogc {
		chart_set_style(this.handle, style);
	}

	void setTable() nothrow @nogc {
	...
	void setTableFont(lxw_chart_font* font) nothrow @nogc {
		chart_set_table_font(this.handle, font);
	}

	void setUpDownBars() nothrow @nogc {
	...
	void setUpDownBarsFormat(lxw_chart_line* upBarLine, 
			...
			lxw_chart_fill* downBarFill) nothrow @nogc 
	{
		chart_set_up_down_bars_format(this.handle, upBarLine, upBarFill,
				downBarLine, downBarFill);
	}

	void setDropLines(lxw_chart_line* line) nothrow @nogc {
		chart_set_drop_lines(this.handle, line);
	}

	void setHighLowLines(lxw_chart_line* line) nothrow @nogc {
		chart_set_high_low_lines(this.handle, line);
	}

	void setSeriesOverlap(byte overlap) nothrow @nogc {
		chart_set_series_overlap(this.handle, overlap);
	}

	void setSeriesGap(ushort gap) nothrow @nogc {
		chart_set_series_gap(this.handle, gap);
	}

	void showBlanksAs(ubyte blanks) nothrow @nogc {
		chart_show_blanks_as(this.handle, blanks);
	}

	void showHiddenData() nothrow @nogc {
		chart_show_hidden_data(this.handle);
	}

	void setRotation(ushort rotation) nothrow @nogc {
